...
//    private final Rect mBounds = new Rect();
     ...
     */
    public DividerItemDecoration(Context context, int orientation, boolean isShowInLastItem, @Nullable Rect dividerMargin) {
        final TypedArray a = context.obtainStyledAttributes(ATTRS);
        mDivider = a.getDrawable(0);
        if (dividerMargin != null) {
            InsetDrawable inset = new InsetDrawable(mDivider, dividerMargin.left, dividerMargin.top, dividerMargin.right, dividerMargin.bottom);
            mDivider = inset;
        }
        if (mDivider == null) {
...
//            final int right = mBounds.right + Math.round(child.getTranslationX());
    ...
    public void getItemOffsets(Rect outRect, View view, RecyclerView parent, RecyclerView.State state) {
        ...
        if (mIsShowInLastItem || itemPosition != itemCount -1) {
            if (mOrientation == VERTICAL) {
